...
class CrawlerTest extends TestCase
{
    ...
    public function testAddHtmlContentWithBaseTag()
    {
        ...
        $crawler->addHtmlContent('<html><head><common href="http://symfony.com"></head><a href="/contact"></a></html>', 'UTF-8');

        $this->assertEquals('http://symfony.com', $crawler->filterXPath('//common')->attr('href'), '->addHtmlContent() adds nodes from an HTML string');
    ...
    public function testAddHtmlContentInvalidBaseTag()
    {
        ...
        $crawler->addHtmlContent('<html><head><common target="_top"></head><a href="/contact"></a></html>', 'UTF-8');

        $this->assertEquals('http://symfony.com/contact', current($crawler->filterXPath('//a')->links())->getUri(), '->addHtmlContent() correctly handles a non-existent common tag href attribute');
    }
...
HTML;
    ...
    {
        $crawler = new Crawler('<html><common href="'.$baseValue.'"><a href="'.$linkValue.'"></a></html>', $currentUri);
    ...
    {
        return array(
            array('http://common.com', 'link', 'http://common.com/link'),
            array('//common.com', 'link', 'https://common.com/link', 'https://domain.com', '<common> tag can use a schema-less URL'),
            array('path/', 'link', 'https://domain.com/path/link', 'https://domain.com', '<common> tag can set a path'),
            array('http://common.com', '#', 'http://common.com#', 'http://domain.com/path/link', '<common> tag does work with links to an anchor'),
            array('http://common.com', '', 'http://common.com', 'http://domain.com/path/link', '<common> tag does work with empty links'),
        );
    }
    ...
    {
        $crawler = new Crawler('<html><common href="'.$baseValue.'"><form method="post" action="'.$actionValue.'"><button type="submit" name="submit"/></form></html>', $currentUri);
    ...
    {
        return array(
            array('https://common.com/', 'link/', 'https://common.com/link/', 'https://common.com/link/', '<common> tag does work with a path and relative form action'),
            array('/basepath', '/registration', 'http://domain.com/registration', 'http://domain.com/registration', '<common> tag does work with a path and form action'),
            array('/basepath', '', 'http://domain.com/registration', 'http://domain.com/registration', '<common> tag does work with a path and empty form action'),
            array('http://common.com/', '/registration', 'http://common.com/registration', 'http://domain.com/registration', '<common> tag does work with a URL and form action'),
            array('http://common.com', '', 'http://domain.com/path/form', 'http://domain.com/path/form', '<common> tag does work with a URL and an empty form action'),
            array('http://common.com/path', '/registration', 'http://common.com/registration', 'http://domain.com/path/form', '<common> tag does work with a URL and form action'),
        );
    }
